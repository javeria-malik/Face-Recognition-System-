...
def check_connected_cameras():
    # Get the indexes and names of connected cameras
    connected_cameras = []
    for index in range(10):  # Check up to 10 camera indexes
        try:
            cap = cv2.VideoCapture(index)
            if cap.isOpened():
                camera_name = f"Camera {index}"
                connected_cameras.append((index, camera_name, cap))
            else:
                cap.release()
...
if cameras:
    print("Number of connected cameras:", len(cameras))
    print("Camera Indexes and Names:")
    for index, name, cap in cameras:
...
#     tasks = [capture_images(class_start_time, camera_index, teacher_name) for class_start_time, camera_index, teacher_name in class_routines]
